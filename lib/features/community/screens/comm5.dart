...
class FinishesPostModel {
  ...
  factory FinishesPostModel.fromMap(
    Map<String, dynamic> map,
    String documentId,
  ) {
    return FinishesPostModel(
      id: documentId,
...
// ==========================================
// 2. Main Community Screen (صفحة مجتمع التشطيبات الرئيسية)
// ==========================================
class FinishesCommunityScreen extends StatefulWidget {
  const FinishesCommunityScreen({super.key});

  @override
  State<FinishesCommunityScreen> createState() =>
      _FinishesCommunityScreenState();
}

class _FinishesCommunityScreenState extends State<FinishesCommunityScreen>
  ...
  // --- دالة مساعدة لعرض صورة البروفايل بأمان ---
          ...
          onPressed: () {
            if (_tabController.index == 1) {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const CreateFinishesQuestionScreen(),
                ),
              );
            } else {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const CreateFinishesPostScreen(),
                ),
              );
            }
          },
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Text(
            "أكبر تجمع للفنيين والمهتمين بأعمال التشطيبات الحديثة في مصر. نتبادل الخبرات والحلول الفنية وأفكار التنفيذ والتصميم.",
  ...
  Widget _buildPostCard(FinishesPostModel post) {
...
class CreateFinishesPostScreen extends StatefulWidget {
  ...
  @override
  State<CreateFinishesPostScreen> createState() =>
      _CreateFinishesPostScreenState();
}

class _CreateFinishesPostScreenState extends State<CreateFinishesPostScreen> {
  ...
  Future<void> _publishPost() async {
    ...
    try {
      ...
      FinishesPostModel newPost = FinishesPostModel(
...
// ==========================================
// 4. Create Finishes Question Screen (طرح سؤال التشطيبات)
// ==========================================
class CreateFinishesQuestionScreen extends StatefulWidget {
  const CreateFinishesQuestionScreen({super.key});

  @override
  State<CreateFinishesQuestionScreen> createState() =>
      _CreateFinishesQuestionScreenState();
}

class _CreateFinishesQuestionScreenState
    extends State<CreateFinishesQuestionScreen> {
  final TextEditingController _contentController = TextEditingController();
  final Color brandBlue = const Color(0xFF0053AC);
  bool _isLoading = false;
  ...
  Future<void> _publishQuestion() async {
    ...
    try {
      FinishesPostModel newQuestion = FinishesPostModel(
        authorName: displayName,
        authorRole: displayRole,
        time: "الآن",
        content: _contentController.text.trim(),
