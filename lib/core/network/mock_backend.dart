...
class _Fake {
  ...
  dynamic noSuchMethod(Invocation inv) {
    ...
    if (n.contains('set') || n.contains('update') || n.contains('delete')) return Future<dynamic>.value(null);
    if (n.contains('where') || n.contains('orderBy') || n.contains('limit') || n.contains('doc') || n.contains('collection') || n.contains('batch')) return _Fake();
    if (n.contains('verifyPhoneNumber') || n.contains('signInWithCredential') || n.contains('signOut')) return Future<dynamic>.value(null);
    if (n.contains('upload') || n.contains('getPublicUrl') || n.contains('storage') || n.contains('from') || n.contains('select') || n.contains('insert') || n.contains('rpc') || n.contains('functions') || n.contains('invoke')) return _Fake();
    return _Fake();
  }
}
...
class MockSetOptions { const MockSetOptions({bool merge = false}); }
class _MockSetOptions extends MockSetOptions { const _MockSetOptions.legacy({bool merge = false}) : super(merge: merge); }
class MockFieldValue { static dynamic serverTimestamp() => DateTime.now(); static dynamic increment(dynamic n) => n; static dynamic arrayUnion(dynamic l) => l; static dynamic arrayRemove(dynamic l) => l; static dynamic delete() => null; }
class _MockFieldValue extends MockFieldValue {}
class FileOptions { const FileOptions({String? cacheControl, bool? upsert, String? contentType}); }

// Mocks for legacy type names so old files still compile
class FirebaseFirestore extends _Fake { static final FirebaseFirestore instance = FirebaseFirestore(); }
class FirebaseAuth extends _Fake { static final FirebaseAuth instance = FirebaseAuth(); }
class FirebaseStorage extends _Fake { static final FirebaseStorage instance = FirebaseStorage(); }
class Supabase extends _Fake { static final Supabase instance = Supabase(); }
class Timestamp {
...
class Filter {
  Filter(dynamic field, {dynamic isEqualTo, dynamic isGreaterThan, dynamic arrayContains, dynamic isNotEqualTo, dynamic whereIn, dynamic arrayContainsAny});
  static dynamic and(dynamic a, [dynamic b, dynamic c, dynamic d, dynamic e]) => Filter('');
  static dynamic or(dynamic a, [dynamic b, dynamic c, dynamic d, dynamic e]) => Filter('');
}
...
class MockPhoneAuthProvider {
  static MockCredential credential({required String verificationId, required String smsCode}) => MockCredential();
}
