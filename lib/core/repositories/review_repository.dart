...
class ReviewRepository {
  ...
  Future<List<Review>> getTechnicianReviews(String technicianId) async {
    ...
    final data =
        (res['data'] as Map<String, dynamic>?)?['reviews'] ??
        res['data'] ??
        res;
    final list = data is List
        ? data
        : (data is Map && data['data'] is List ? data['data'] : []);
    final raw = data is Map && data['reviews'] is List
        ? data['reviews']
        : (data is List ? data : []);
    final arr =
        (res['data'] is List
    ...
    final actual = (res['data'] is List
        ? res['data']
        : (res['data'] is Map ? (res['data']['reviews'] ?? res['data']) : []));
    // Fallback to parsing correctly
    final list2 = actual is List ? actual : [];
    return (list2 as List)
        .map((e) => Review.fromJson(_normalize(Map<String, dynamic>.from(e))))
        .toList();
  }
  ...
  Future<double> getTechnicianAverageRating(String technicianId) async {
    ...
    final data = res['data'];
    if (data is Map && data['avg'] != null)
      return (data['avg'] as num).toDouble();
    if (data is Map && data['average'] != null)
