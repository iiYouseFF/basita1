...
class AppointmentRepository {
  ...
  Map<String, dynamic> _normalize(Map<String, dynamic> j) => {
        ...
        'technician_longitude': j['technicianLongitude'] ?? j['technician_longitude'],
  ...
  Future<Appointment> createAppointment({
    ...
    final res = await _api.post('/appointments', body: {
      'requestId': requestId,
      'clientId': clientId,
      ...
      if (technicianLatitude != null) 'technicianLatitude': technicianLatitude,
      if (technicianLongitude != null) 'technicianLongitude': technicianLongitude,
      if (estimatedDuration != null) 'estimatedDuration': estimatedDuration,
      if (price != null) 'price': price,
      if (notes != null) 'notes': notes,
    });
    final data = (res['data'] as Map<String, dynamic>?)?['appointment'] ?? res['data'] ?? res;
    return Appointment.fromJson(_normalize(Map<String, dynamic>.from(data as Map)));
  }

  Future<List<Appointment>> getUserAppointments(String userId) async {
    final res = await _api.get('/appointments', query: {'userId': userId});
    final list = _extractList(res);
    return list.map((e) => Appointment.fromJson(_normalize(Map<String, dynamic>.from(e)))).toList();
  }
  ...
  Future<Appointment?> getAppointmentByRequestId(String requestId) async {
    final res = await _api.get('/appointments', query: {'requestId': requestId});
    final list = _extractList(res);
    if (list.isEmpty) return null;
    return Appointment.fromJson(_normalize(Map<String, dynamic>.from(list.first)));
  }
  ...
  Future<Appointment> upsertAppointmentOnAccept({
    ...
    final res = await _api.post('/appointments/upsert-on-accept', body: {
      'requestId': requestId,
      'clientId': clientId,
      'technicianId': technicianId,
      'serviceType': serviceType,
    ...
    final data = (res['data'] as Map<String, dynamic>?)?['appointment'] ?? res['data'] ?? res;
    if (data is Map) return Appointment.fromJson(_normalize(Map<String, dynamic>.from(data)));
    return createAppointment(
      ...
      appointmentTime: '${DateTime.now().hour.toString().padLeft(2, '0')}:${DateTime.now().minute.toString().padLeft(2, '0')}',
  ...
  Future<void> completeAppointmentAndSetLocations({
    ...
    await _api.patch('/appointments/by-request/$requestId/complete', body: {
  ...
  Future<void> updateAppointmentStatus(String appointmentId, String status) async {
    await _api.patch('/appointments/$appointmentId/status', body: {'status': status});
  }
  ...
  Future<void> updateAppointmentLocation({
    ...
    await _api.patch('/appointments/$appointmentId/location', body: {
      'role': role,
      'latitude': latitude,
      'longitude': longitude,
    });
  ...
  Future<List<Appointment>> getTechnicianAppointments(String technicianId) async {
    final res = await _api.get('/appointments', query: {'technicianId': technicianId});
    final list = _extractList(res);
    return list.map((e) => Appointment.fromJson(_normalize(Map<String, dynamic>.from(e)))).toList();
  }
  ...
  List<dynamic> _extractList(Map<String, dynamic> res) {
    ...
    if (data is Map && data['appointments'] is List) return data['appointments'];
    return [];
  }
}
