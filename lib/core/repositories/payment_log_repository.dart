...
class PaymentLogRepository {
  ...
  Future<PaymentLog> logPayment({
    ...
    final data =
        (res['data'] as Map<String, dynamic>?)?['payment'] ??
        (res['data'] as Map<String, dynamic>?)?['paymentLog'] ??
        res['data'] ??
        res;
    if (data is Map<String, dynamic>)
      return PaymentLog.fromJson(_normalize(data));
    return PaymentLog(
  ...
  Map<String, dynamic> _normalize(Map<String, dynamic> j) => {
    'id': j['id'] ?? '',
    'firebase_user_id':
        j['userId'] ?? j['user_id'] ?? j['firebase_user_id'] ?? '',
    'firebase_request_id':
        j['requestId'] ?? j['request_id'] ?? j['firebase_request_id'] ?? '',
  ...
  Future<List<PaymentLog>> getUserPayments(String userId) async {
    ...
    final list = data is List
        ? data
        : (data is Map && data['payments'] is List ? data['payments'] : []);
    return (list as List)
        .map(
          (e) => PaymentLog.fromJson(_normalize(Map<String, dynamic>.from(e))),
        )
        .toList();
  }
