...
class SupportTicketRepository {
  ...
  Future<List<SupportTicket>> getUserTickets(String userId) async {
    ...
    final list = data is List
        ? data
        : (data is Map && data['tickets'] is List ? data['tickets'] : []);
    return (list as List)
        .map(
          (e) =>
              SupportTicket.fromJson(_normalize(Map<String, dynamic>.from(e))),
        )
        .toList();
  }
  ...
  Future<SupportTicket> createTicket({
    ...
    final data =
        (res['data'] as Map<String, dynamic>?)?['ticket'] ?? res['data'] ?? res;
    return SupportTicket.fromJson(
